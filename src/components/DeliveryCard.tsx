...
const DeliveryCard = ({ order, fullWidth = false }: { order: Order; fullWidth?: boolean }) => {
  const tw = useTailwind();
  const nav = useNavigation<CustomerViewNavProp>();
  return (
    <Card
      containerStyle={[
        tw(fullWidth ? 'mx-0 my-1' : 'rounded-lg my-2'),
        {
    ...
    >
      <View style={fullWidth && { height: '100%', width: '100%' }}>
        <View>
        ...
        <MapView
          ...
          style={[tw('mt-5 w-full'), fullWidth ? { height: '100%' } : { height: 200 }]}
